...
cDecoderNEW::cDecoderNEW(const char *recDir, int threadsParam, const bool fullDecodeParam, char *hwaccel, cIndex *indexParam) {
    dsyslog("cDecoderNEW::cDecoder(): create new decoder object");
    // recording directory
    if (!recordingDir) {
    ...
    dsyslog("cDecoderNEW::cDecoder(): init with %d threads, full decode %d, hwaccel %s", threads, fullDecode, hwaccel);
...
// init hwaccel device
    if (hwaccel[0] != 0) {
        ...
        if (hwDeviceType == AV_HWDEVICE_TYPE_NONE) {
            ...
            dsyslog("cDecoderNEW::cDecoder(): hardware acceleration device type %s found", av_hwdevice_get_type_name(hwDeviceType));
            useHWaccel = true;
        }
    }
}
...
bool cDecoderNEW::ReadNextFile() {
    ...
    dsyslog("cDecoderNEW:::ReadNextFile(): search next file %s", filename);
    bool ret = false;
    struct stat buffer;
    ...
    for (unsigned int streamIndex = 0; streamIndex < avctx->nb_streams; streamIndex++) {
        ...
        if (useHWaccel && IsVideoStream(streamIndex)) {
            for (int i = 0; ; i++) {
                ...
                if (!config) {
                    dsyslog("cDecoderNEW::InitDecoder(): codec %s does not support hardware acceleration device type %s", codec->name, av_hwdevice_get_type_name(hwDeviceType));
                    useHWaccel= false;
                    break;
                }
                ...
                if ((config->methods & AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX) && (config->device_type == hwDeviceType)) {
                    ...
                    dsyslog("cDecoderNEW::InitDecoder(): codec %s supported by hardware acceleration device type %s with pixel format %d %s", codec->name, av_hwdevice_get_type_name(hwDeviceType), hw_pix_fmt, av_get_pix_fmt_name(hw_pix_fmt));
                    break;
                }
            }
        }
        ...
        if (useHWaccel && (hw_pix_fmt != AV_PIX_FMT_NONE) && IsVideoStream(streamIndex)) {
            ...
            if (ret >= 0) {
                dsyslog("cDecoderNEW::InitDecoder(): linked hardware acceleration device %s successful for stream %d", av_hwdevice_get_type_name(hwDeviceType), streamIndex);
                codecCtxArray[streamIndex]->hw_device_ctx = av_buffer_ref(hw_device_ctx);
                if (codecCtxArray[streamIndex]->hw_device_ctx) {
                    dsyslog("cDecoderNEW::InitDecoder(): hardware device reference created successful for stream %d", streamIndex);
                    codecCtxArray[streamIndex]->get_format = get_hw_format;
                    isyslog("use hardware acceleration for stream %d", streamIndex);
                }
                else {
                    dsyslog("cDecoderNEW::InitDecoder(): hardware device reference create failed for stream %d, fall back to software decoder", streamIndex);
                    hwPixelFormat = AV_PIX_FMT_NONE;
                    useHWaccel = false;
                }
            }
            else {
                dsyslog("cDecoderNEW::InitDecoder(): failed to link hardware acceleration device %s. Error code: %s", av_hwdevice_get_type_name(hwDeviceType), av_err2str(ret));
                useHWaccel = false;
            }
        }
...
int cDecoderNEW::GetVideoFrameRate() {
    ...
    if ((GetFileNumber() == 1) && IsInterlacedVideo() && (GetVideoType() == MARKAD_PIDTYPE_VIDEO_H264) &&
        ...
        dsyslog("cDecoderNEW::GetVideoFrameRate() use real frame rate %d", frameRate);
    }
    else {
        frameRate = GetVideoAvgFrameRate();
        dsyslog("cDecoderNEW::GetVideoFrameRate() use avg frame rate %d", frameRate);
    }
    return frameRate;
...
bool cDecoderNEW::DecodeNextFrame(const bool audioDecode) {
    while (true) {
        ...
        case 0:  // we had successful send last packet and we can try to send next packet
            ...
            if(ReadNextPacket()) {        // got a packet
                ...
                dsyslog("cDecoderNEW::DecodeNextFrame(): packet (%d): end of all ts files reached", packetNumber);
        ...
        case -EAGAIN: // full queue at last send, send now again without read from input
            ...
            break;
        case AVERROR_EOF:
            dsyslog("cDecoderNEW::DecodeNextFrame(): packet (%5d): end of file (AVERROR_EOF)", packetNumber);
            return false;
            break;
        default:
            esyslog("cDecoderNEW::DecodeNextFrame(): packet (%5d): unexpected state of decoder send queue rc = %d: %s", packetNumber, decoderSendState, av_err2str(decoderSendState));
            return false;
            break;
        }
// now queue is filled, receive a frame from decoder
        ...
        case -EIO:        // end of file
            dsyslog("cDecoderNEW::DecodeNextFrame(): frame  (%5d): I/O error (EIO)", frameNumber);
            break;
        case AVERROR_EOF:        // end of file
            dsyslog("cDecoderNEW::DecodeNextFrame(): frame  (%5d): end of file (AVERROR_EOF)", frameNumber);
            break;
        default:
            esyslog("cDecoderNEW::DecodeNextFrame(): frame  (%5d): unexpected return code from decoder rc = %d: %s", frameNumber, rc, av_err2str(rc));
            return false;
            break;
        }
    }
...
int cDecoderNEW::SendPacketToDecoder(const bool flush) {
    ...
    if (flush) {
        ...
        dsyslog("cDecoderNEW::SendPacketToDecoder(): packet (%d), stream %d: flush queue send", packetNumber, avpkt.stream_index);
    }
...
#endif
        default:
            esyslog("cDecoderNEW::SendPacketToDecoder(): packet (%d), stream %d: avcodec_send_packet failed with rc=%d: %s", packetNumber, avpkt.stream_index, rc, av_err2str(rc));
            break;
        }
...
int cDecoderNEW::ReceiveFrameFromDecoder() {
    ...
    if (IsVideoPacket()) {
        ...
        if (IsAudioPacket()) {
            ...
            if (ret < 0) {
                dsyslog("cDecoderNEW::ReceiveFrameFromDecoder(): av_channel_layout_copy failed, rc = %d", ret);
                av_frame_unref(&avFrame);
                return AVERROR_EXIT;
            }
#else
            ...
            if (IsSubtitlePacket()) { // do not decode subtitle, even on fullencode use it without reencoding
                ...
                dsyslog("cDecoderNEW::ReceiveFrameFromDecoder(): stream %d type not supported", avpkt.stream_index);
                av_frame_unref(&avFrame);
                return AVERROR_EXIT;
            }
        }
    ...
    if (!useHWaccel) {  // we do not need a buffer for avcodec_receive_frame() if we use hwaccel
        ...
        if (rc != 0) {
            ...
            dsyslog("cDecoderNEW::ReceiveFrameFromDecoder(): stream index %d: av_frame_get_buffer failed: %s", avpkt.stream_index, errTXT);
            av_frame_unref(&avFrame);
            return AVERROR_EXIT;
        }
    }
    ...
    if (rc < 0) {
        ...
        case AVERROR(EAGAIN):   // frame not ready
            dsyslog("cDecoderNEW::ReceiveFrameFromDecoder(): frame (%d): not ready, retry later", frameNumber);
            break;
        case AVERROR(EINVAL):
            esyslog("cDecoderNEW::ReceiveFrameFromDecoder(): frame (%d): avcodec_receive_frame error EINVAL", frameNumber);
            break;
        case -EIO:        // end of file
            dsyslog("cDecoderNEW::ReceiveFrameFromDecoder(): frame (%5d): I/O error (EIO)", frameNumber);
            break;
        case AVERROR_EOF:        // end of file
            dsyslog("cDecoderNEW::ReceiveFrameFromDecoder(): frame (%5d): end of file (AVERROR_EOF)", frameNumber);
            break;
        default:
            esyslog("cDecoderNEW::ReceiveFrameFromDecoder(): frame (%d): avcodec_receive_frame decode failed with return code %d", frameNumber, rc);
            break;
        }
        av_frame_unref(&avFrame);
        return rc;
    ...
    if (useHWaccel) {
        ...
        if (rc < 0 ) {
            ...
            case -EIO:        // end of file
                ...
                break;
            default:
                esyslog("cDecoderNEW::ReceiveFrameFromDecoder(): frame (%d): av_hwframe_transfer_data rc = %d: %s", frameNumber, rc, av_err2str(rc));
                break;
            }
    ...
    if (avFrame.decode_error_flags != 0) {
        ...
        dsyslog("cDecoderNEW::ReceiveFrameFromDecoder(): frame (%d): decoding from stream %d failed: decode_error_flags %d, decoding errors %d", frameNumber, avpkt.stream_index, avFrame.decode_error_flags, decodeErrorCount);
        av_frame_unref(&avFrame);
...
bool cDecoderNEW::IsVideoFrame() {
    if (!avctx) return false;
    if (avFrame.pict_type == AV_PICTURE_TYPE_NONE) return false;
    return true;
}

bool cDecoderNEW::IsVideoIFrame() {
    if (!avctx) return false;
...
/* for future use
int cDecoderNEW::GetVideoPacketNumber() const {
    return packetNumber;
}
*/


int cDecoderNEW::GetVideoFrameNumber() const {
    return frameNumber;
}
...
sAspectRatio *cDecoderNEW::GetFrameAspectRatio() {
