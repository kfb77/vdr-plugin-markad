...
void cMarkAdStandalone::AddMark(sMarkAdMark *mark) {
    ...
    case MT_NOLOWERBORDERSTART:
        if (asprintf(&comment, "end   lower border (%d)*", mark->position) == -1) comment = NULL;
        if (comment) {
            ALLOC(strlen(comment)+1, "comment");
        }
        break;
    case MT_NOLOWERBORDERSTOP:
        if (asprintf(&comment, "start lower border (%d) ", mark->position) == -1) comment = NULL;
        if (comment) {
            ALLOC(strlen(comment)+1, "comment");
        }
        break;
    ...
    while (mark) {
        ...
        if ((mark->type & 0x0F) == MT_START) {
            ...
            if (startBefore) {
                ...
                case MT_LOGOSTART:
                    ...
                    else if ((criteria.LogoFadeInOut(macontext.Info.ChannelName) & FADE_IN) && (lengthBefore > 40)) maxBefore = 4960;
                    else                                                                                            maxBefore = 3020;
                    break;
                case MT_CHANNELSTART:
                    maxBefore = 1240;
        ...
        if ((mark->type & 0x0F) == MT_STOP) {
            ...
            if (stopAfter) {  // move even to same position to prevent scene change for move again
                ...
                switch (mark->type) {
                case MT_ASSUMEDSTOP:
                    if (lengthAfter >= 120) maxAfter = 98720;
                    else                    maxAfter = 25119;
                    break;
                case MT_LOGOSTOP:
