...
#define MAX_BEFORE_CHANNEL 336   // do not increase, will miss end in double episodes with 6 channels
    ...
    if (end) {
        ...
        if (end) {
            ...
            if ((diffAssumed > MAX_BEFORE_CHANNEL) && channelStartAfter) {
                dsyslog("cMarkAdStandalone::Check_CHANNELSTOP(): last MT_CHANNELSTOP (%d) too far before assumed stop and MT_CHANNELSTART (%d) after exists", end->position, channelStartAfter->position);
                end = NULL;
            }
...
#define MAX_LOGO_BEFORE_ASSUMED 105
...
// MT_LOGOSTOP (  204) ->  28460ms -> MT_SOUNDSTOP (1627) ->   240ms -> MT_SOUNDSTART ( 1639) ->  1840ms -> MT_LOGOSTART ( 1731) -> ZDF HD, Wetter before broadcast (conflict)
                    if ((logoStopSilenceStart < 28460) &&
                            (silenceStartSilenceStop >= 160) && (silenceStartSilenceStop <=  440) &&
        ...
        if (silenceStop) {
            ...
            if (silenceStart) {
                ...
                if (logoStop) {
                    ...
                    if ((logoStopSilenceStart <= 42040) && (silenceStartLogoStart <= 360) && (logoStartSilenceStop <= 40)) {
                        dsyslog("cMarkAdStandalone::HaveSilenceSeparator(): logo start mark (%d): silence sequence is valid", mark->position);
                        return true;
                    }
                    dsyslog("cMarkAdStandalone::HaveSilenceSeparator(): logo start mark (%d): silence sequence is invalid", mark->position);
...
//                                      MT_LOGOSTOP ( 93602) ->  120ms -> MT_SOUNDSTOP ( 93605) ->  120ms -> MT_SOUNDSTART ( 93608) -> 40160ms -> MT_LOGOSTART ( 94612) -> VOXup
...
//
// invalid sequence
//                                      MT_LOGOSTOP ( 42715) -> 4840ms -> MT_SOUNDSTOP ( 42836) ->  360ms -> MT_SOUNDSTART ( 42845) -> 94760ms -> MT_LOGOSTART ( 45214)
...
// MT_LOGOSTART ( 82878) ->   81600s -> MT_LOGOSTOP ( 84918) -> 3880ms -> MT_SOUNDSTOP ( 85015) ->  120ms -> MT_SOUNDSTART ( 85018) -> 20120ms -> MT_LOGOSTART ( 85521)
                        ...
                        if ((logoStartLogoStop >= 10400) &&            // possible end of info logo before
                                (logoStopSilenceStart <= 160) &&       // silence start short after end mark
                                (silenceStartSilenceStop >= 80) && (silenceStartSilenceStop <= 320) &&
                                (silenceStopLogoStart <= 40160)) {
...
bool cMarkAdStandalone::HaveBlackSeparator(const cMark *mark) {
    ...
    if (mark->type == MT_LOGOSTART) {
        ...
        cMark *blackStop = blackMarks.GetPrev(mark->position + 1, MT_NOBLACKSTART);  // black screen end can be on same position as logo start
        if (blackStop) {
        ...
        if (blackStop) {
            ...
            if (blackStart) {
                ...
                if (stopBefore) {
                    ...
                    dsyslog("cMarkAdStandalone::HaveBlackSeparator(): MT_LOGOSTOP (%5d)-> %4dms -> MT_NOBLACKSTOP (%d) -> %4dms -> MT_LOGOSTART (%5d) -> %4dms -> MT_NOBLACKSTART (%5d)", stopBefore->position, diffLogoStopBlackStart, blackStart->position, diffBlackStartLogoStart, mark->position, diffLogoStartBlackStop, blackStop->position);
                    // valid example
                    ...
                    if ((diffLogoStopBlackStart  >= 0) && (diffLogoStopBlackStart  <= 79080) &&
                            (diffBlackStartLogoStart >= 0) && (diffBlackStartLogoStart <=    40) &&
                            (diffLogoStartBlackStop  >= 0) && (diffLogoStartBlackStop  <=   440)) {
                        dsyslog("cMarkAdStandalone::HaveBlackSeparator(): black screen sequence is valid");
                        return true;
                    }
                    else dsyslog("cMarkAdStandalone::HaveBlackSeparator(): black screen sequence is invalid");
        ...
        if (startAfter) {
            ...
            if (blackStart) {
                ...
                if (blackStop) {
                    ...
                    dsyslog("cMarkAdStandalone::HaveBlackSeparator(): MT_NOBLACKSTOP (%5d) -> %4dms -> MT_LOGOSTOP (%5d) -> %4dms -> MT_NOBLACKSTART (%5d) -> %5dms -> MT_LOGOSTART (%5d)", blackStart->position, diffBlackStartLogoStop, mark->position, diffLogoStopBlackStop, blackStop->position, diffBlackStopLogoStart, startAfter->position);
// valid sequence
...
// MT_NOBLACKSTOP (84098) -> 1400ms -> MT_LOGOSTOP (84133) ->   40ms -> MT_NOBLACKSTART (84134) ->  1560ms -> MT_LOGOSTART (84173) -> RTL2
                    if ((diffBlackStartLogoStop >= 360) && (diffBlackStartLogoStop <= 1400) &&
                            (diffLogoStopBlackStop  >=   40) && (diffLogoStopBlackStop  <=   280) &&
                            (diffBlackStopLogoStart >= 1560) && (diffBlackStopLogoStart <= 18080)) {
        ...
        if (blackStop) {
            ...
            if (blackStart) {
                ...
                //
                // invalid example
                // MT_NOBLACKSTOP  (84646) -> 160ms -> MT_NOBLACKSTART  (84650) ->  480ms -> MT_LOGOSTOP (84662)    -> pattern in blackground from preview (RTLZWEI)
                if ((lengthBlack >= 40) && (distLogoStop > 480) && (distLogoStop <= 1340)) {
                    dsyslog("cMarkAdStandalone::HaveBlackSeparator(): logo stop mark (%d): black screen sequence is valid", mark->position);
                    return true;
                }
                dsyslog("cMarkAdStandalone::HaveBlackSeparator(): logo stop mark (%d): black screen sequence is invalid", mark->position);
...
cMark *cMarkAdStandalone::Check_LOGOSTART() {
    ...
    if (evaluateLogoStopStartPair) {
        ...
        while (!begin && lStart) {
            ...
            if ((diffAssumed < -307) || (diffAssumed > 307)) break;  // chnaged from 300 to 307
            if (status == STATUS_YES) {
    ...
    while (!begin && lStart) {
        int diffAssumed = (lStart->position - iStartA)          / macontext.Video.Info.framesPerSecond;
#define MAX_AFTER_ASSUMED 335    // changed from 293 to 335
        LogSeparator();
    ...
    while (!begin && lStart) {
        ...
        if (diffAssumed < -164) {  // later start mark can be start of first ad, changed from -162 to -164
                    ...
                    if (stopBefore && startAfter) {  // if advertising before is long this is the really the next start mark
                        ...
                        if ((lengthAdBefore >= 800) || (lengthAdAfter >= 360)) {  // check if we have ad before or after preview. if not it is a logo detection failure
                            ...
                            if (((lengthAdBefore >= 120) && (lengthAdBefore <= 585000) && (lengthAdAfter >= 200)) || // if advertising before is long this is the next start mark
                                ...
                                if (lengthPreview <= 143) {  // changed from 143 to 154, longest preview found
            ...
            if (stopAfter) {  // move even to same position to prevent scene change for move again
                ...
                case MT_MOVEDSTOP:
                    ...
                    case MT_VPSSTOP:
                        ...
                        else if (lengthAfter > 120)                       maxAfter = 231480;
        ...
        if ((mark->type & 0x0F) == MT_START) {
            ...
            if (soundStartBefore && (soundStartBefore->position != mark->position)) { // do not move to same frame
                ...
                case MT_MOVEDSTART:
                    switch (mark->newType) {
                    case MT_VPSSTART:
                        // rule 1: silence before preview and before broadcast start, no black border
                        if (!lowerBefore && (diffBefore >= 7440) && (lengthBefore >= 480) && (diffAfter <= 2400) && (lengthAfter <= 120)) diffBefore = INT_MAX;

                        if (lowerBefore) maxBefore = 51440;   // lower border short before silence
            ...
            if (!moved && soundStartAfter) {
                ...
                case MT_MOVEDSTART:
                    switch (mark->newType) {
                    case MT_VPSSTART:
                        if (lengthAfter >= 1200)                                maxAfter = 231440;
                        else if ((lengthAfter >=  120) && (lengthAfter <= 240)) maxAfter = 213920;   // typical silence length at start
                        else                                                    maxAfter =  26599;
                        break;
                    default:
                        maxAfter = 0;
        ...
        if ((mark->type & 0x0F) == MT_STOP) {
            ...
            if (soundStopAfter) {
                ...
                if (soundStartAfter) {
                    ...
                    if (black) blackAfter = true;
                    lengthAfter = 1000 * (soundStartAfter->position - soundStopAfter->position) / macontext.Video.Info.framesPerSecond;
                    dsyslog("cMarkAdStandalone::SilenceOptimization(): stop  mark (%6d): silence from (%5d) to (%5d) %8dms after,  length %4dms, black screen %d", mark->position, soundStopAfter->position, soundStartAfter->position, diffAfter, lengthAfter, blackAfter);
                }
            }
