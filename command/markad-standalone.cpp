...
#define MAX_LOGO_END_MARK_FACTOR 2.7 // changed from 3 to 2.7 to prevent too early logo stop marks
    if (!end) {  // try logo stop mark
        ...
        while (true) {
            ...
            if (end) {
                int iStopDelta = (end->position - iStopA) / macontext.Video.Info.framesPerSecond;
...
// try blacksceen mark
    ...
    // logo fade in/out
    if (macontext.Info.ChannelName) {  // macontext.Info.ChannelName == NULL can happen if the VDR info file is missing
        if  (strcmp(macontext.Info.ChannelName, "TELE_5")         == 0)  blackscreenRange = 4720;  // this channel have fading out logo, so we need more range
