...
#define MAX_LOGO_BEFORE_ASSUMED 82   // changed from 98 to 82, do not increase, we will lost last part
        ...
        if (blackStop) {
            ...
            if (stopBefore) {
                ...
                if (blackStart) {
                    ...
                    if ((diffBlackStartLogoStop <= 4760) && (diffLogoStopBlackStop <= 5000) && (diffBlackStopLogoStart <= 2360)) {
...
// MT_LOGOSTOP (5279)-> 124400ms -> MT_NOBLACKSTOP (8389) ->   40ms -> MT_NOBLACKSTART (8390) -> 2600ms -> MT_LOGOSTART (8455)  Nickelodeon
                    if ((diffLogoStopBlackStart <= 124400) && (diffBlackStartBlackStop >= 40) && (diffBlackStopLogoStart <= 2600)) {
                        dsyslog("cMarkAdStandalone::HaveBlackSeparator(): black screen sequence is valid");
                        return true;
                    }
                    else dsyslog("cMarkAdStandalone::HaveBlackSeparator(): black screen sequence is invalid");
...
cMark *cMarkAdStandalone::Check_LOGOSTART() {
    ...
    if (evaluateLogoStopStartPair) {
        ...
        while (!begin && lStart) {
            LogSeparator(false);
            dsyslog("cMarkAdStandalone::Check_LOGOSTART(): check logo start mark (%d) based on closing credits before logo start", lStart->position);
        ...
        while (!begin) {
            ...
            if (!lStart) break;
            LogSeparator(false);
            dsyslog("cMarkAdStandalone::Check_LOGOSTART(): check logo start mark (%d) based on closing credits after logo start", lStart->position);
            int status = evaluateLogoStopStartPair->GetIsClosingCreditsBefore(lStart->position);
    ...
    while (!begin && lStart) {
        int diffAssumed = (lStart->position - iStartA)          / macontext.Video.Info.framesPerSecond;
#define MAX_AFTER_ASSUMED 293    // changed from 266 to 293
        LogSeparator();
    ...
    while (!begin && lStart) {
        ...
        if (diffAssumed <= -162) {  // later start mark can be start of first ad, changed from -296 to -162
            ...
            if (startBefore) {
                ...
                case MT_MOVEDSTART:
                    ...
                    case MT_VPSSTART:
                        ...
                        // rule 2: first black screen in broadcast before, second blackscreen is start of broadcast, no silence around
                        else if (!silenceBefore && !silenceAfter &&
                                 (diffBefore >= 48040) && (diffBefore <= 74080) && (lengthBefore >= 1000) && (lengthBefore <= 1520) &&
                                 (diffAfter  >= 13280) && (diffAfter  <= 26560) && (lengthAfter  >=   80) && (lengthAfter  <= 2400)) diffBefore = INT_MAX;
        ...
        if ((mark->type & 0x0F) == MT_STOP) {
            ...
            if (stopAfter) {  // move even to same position to prevent scene change for move again
                ...
                case MT_MOVEDSTOP:
                    switch (mark->newType) {
                    case MT_VPSSTOP:
                        // rule 1: silence before and after, first blackscreen is end mark, second is after preview or in next broadcast
                        if (silenceBefore && silenceAfter && (diffBefore <= 6360) && (diffAfter >= 12560)) diffAfter = INT_MAX;

                        // rule 2: no silence before or after, first blackscreen is end mark, second is after preview or in next broadcast
                        else if (!silenceBefore && !silenceAfter && (diffBefore <= 116120) && (lengthBefore >= 200) && (diffAfter >= 8500) && (lengthAfter <= 280)) diffAfter = INT_MAX;

                        // rule 3: valid black screen with silence around before
                        else if (silenceBefore && !silenceAfter && (diffBefore <= 6260) && (diffAfter >= 4180)) diffAfter = INT_MAX;

                        if (lengthAfter >= 80) maxAfter = 231480;
