        ...
        if (silenceStop) {
            ...
            if (silenceStart) {
                ...
                if (logoStop) {
                    ...
                    int silenceStartSilenceStop = 1000 * (silenceStop->position  - silenceStart->position) / decoder->GetVideoFrameRate();
                    int silenceStopLogoStart    = 1000 * (mark->position         - silenceStop->position)  / decoder->GetVideoFrameRate();
                    dsyslog("cMarkAdStandalone::HaveSilenceSeparator(): MT_LOGOSTOP (%6d) -> %6dms -> MT_SOUNDSTOP (%6d) -> %4dms -> MT_SOUNDSTART (%6d) -> %4dms -> MT_LOGOSTART (%6d) -> %s", logoStop->position, logoStopSilenceStart, silenceStart->position, silenceStartSilenceStop, silenceStop->position, silenceStopLogoStart, mark->position, macontext.Info.ChannelName);
// valid example
// MT_LOGOSTOP (  7444) ->  24040ms -> MT_SOUNDSTOP (  8045) ->   80ms -> MT_SOUNDSTART (  8047) ->  760ms -> MT_LOGOSTART (  8066) -> Disney_Channel
//
// invalid example
// MT_LOGOSTOP (  2028) -> 104400ms -> MT_SOUNDSTOP (  4638) ->  400ms -> MT_SOUNDSTART (  4648) ->  600ms -> MT_LOGOSTART (  4663) -> Comedy_Central, silence before first ad
                    if ((logoStopSilenceStart        >=  24040) && (logoStopSilenceStart < 104400) &&
...
// MT_LOGOSTART ( 3833) -> 480ms -> MT_LOGOSTOP ( 3845) -> 480ms -> MT_LOGOSTART ( 3857)
        if ((diffStop2Start2 >= 720) && (diffStop2Start2 <=  1120)   &&  // change from logo to closing logo
                (diffStart2Stop1 >= 6400) && (diffStart2Stop1 <= 8880) &&  // closing logo deteted as logo
...
// check for better end mark not very far away from assuemd end
    ...
    while (moreMarks) {
        ...
        dsyslog("cMarkAdStandalone::CheckMarks(): MT_START (%6d) -> %4ds -> MT_STOP (%6d) |%4ds| -> %3ds -> MT_START (%6d) -> %3ds -> MT_STOP (%6d) |%3ds| -> %s", prevStartMark->position, prevBroadcast, prevStopMark->position, diffPrevStopAssumed, lastAd, lastStartMark->position, lastBroadcast, lastStopMark->position, diffLastStopAssumed, macontext.Info.ChannelName);
        ...
        if ((mark->type & 0x0F) == MT_START) {
            ...
            if (stopBlackBefore) {
                ...
                if (startBlackBefore) {
                    ...
                    cMark *silence = silenceMarks.GetAround(decoder->GetVideoFrameRate(), startBlackBefore->position, MT_SOUNDCHANGE, 0xF0);         // around black screen start
            ...
            if (stopBlackAfter) {
                ...
                if (startBlackAfter) {
                    ...
                    // check if there is silence around black screen
                    cMark *silence = silenceMarks.GetAround(decoder->GetVideoFrameRate(), startBlackAfter->position, MT_SOUNDCHANGE, 0xF0);         // around black screen start
        ...
        if ((mark->type & 0x0F) == MT_STOP) {
            ...
            if (startBlackAfter) {
                ...
                if (stopBlackAfter) {
                    lengthAfter = 1000 * (stopBlackAfter->position - startBlackAfter->position) / decoder->GetVideoFrameRate();
                    // check if there is silence around black screen
                    cMark *silence = silenceMarks.GetAround(decoder->GetVideoFrameRate(), startBlackAfter->position, MT_SOUNDCHANGE, 0xF0);
                    if (silence) silenceAfter = true;
        ...
        if ((mark->type & 0x0F) == MT_START) {
            ...
            if (soundStartBefore) {
                ...
                if (soundStopBefore) {
                    ...
                    dsyslog("cMarkAdStandalone::SilenceOptimization(): start mark (%6d): silence from (%6d) %" PRId64 " to (%6d) %" PRId64 ", %8dms before, length %4dms, black %d", mark->position, soundStopBefore->position, soundStopBefore->pts, soundStartBefore->position, soundStartBefore->pts, diffBefore, lengthBefore, blackBefore);
                }
            }
            if (soundStartAfter) {
