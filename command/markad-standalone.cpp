...
// MT_LOGOSTOP (72210) ->  1400ms -> MT_NOBLACKSTOP (72245) ->   200ms ->  MT_NOBLACKSTART (72250) -> 104320ms -> MT_LOGOSTART (74858)  TLC (conflict)
...
// MT_LOGOSTOP (81485) ->  4040ms -> MT_NOBLACKSTOP (81586) ->   160ms ->  MT_NOBLACKSTART (81590) ->  95920ms -> MT_LOGOSTART (83988) -> RTLZWEI, sequence in preview
                    if ((diffLogoStopBlackStart <= 4240) && (diffBlackStartBlackStop >= 120) && (diffBlackStartBlackStop < 2920) && (diffBlackStopLogoStart < 95920)) {
                        dsyslog("cMarkAdStandalone::HaveBlackSeparator(): logo stop mark (%d): black screen sequence is valid", mark->position);
                        return true;
                    }
                    dsyslog("cMarkAdStandalone::HaveBlackSeparator(): logo stop mark (%d): black screen sequence is invalid", mark->position);
    ...
    while (mark) {
        ...
        if ((mark->type & 0x0F) == MT_START) {
            ...
            if (soundStartBefore) {
                ...
                cMark *blackLowerStart = blackMarks.GetPrev(soundStartBefore->position, MT_NOLOWERBORDERSTOP);
                if (blackLowerStart) {
                    cMark *blackLowerStop = blackMarks.GetNext(blackLowerStart->position, MT_NOLOWERBORDERSTART);
                    if (blackLowerStop) {
                        int diffBlackLowerBefore = 1000 * (soundStartBefore->position - blackLowerStop->position) / macontext.Video.Info.framesPerSecond;
                        dsyslog("cMarkAdStandalone::SilenceOptimization(): start mark (%6d): lower border before start (%d) end (%d), %dms before", mark->position, blackLowerStart->position, blackLowerStop->position, diffBlackLowerBefore);
            ...
            if (soundStartBefore && (soundStartBefore->position != mark->position)) { // do not move to same frame
                ...
                case MT_MOVEDSTART:
                    ...
                    case MT_VPSSTART:
                        ...
                        if (blackLowerBefore) maxBefore = 51440;   // lower border short before silence
        ...
        if ((mark->type & 0x0F) == MT_STOP) {
            ...
            if (soundStopBefore) {
                ...
                const cMark *blackLowerStart = blackMarks.GetPrev(soundStopBefore->position, MT_NOLOWERBORDERSTOP);
                if (blackLowerStart) {
                    const cMark *blackLowerStop = blackMarks.GetNext(blackLowerStart->position, MT_NOLOWERBORDERSTART);
            ...
            if (!moved && soundStopBefore) {
                ...
                case MT_MOVEDSTOP:
                    switch (mark->newType) {
                    case MT_VPSSTOP:
                        if (blackLowerBefore) maxBefore = 57280;  // lower border around sound stop
        ...
        if ((mark->type & 0x0F) == MT_START) {
            ...
            if (!moved && sceneStartAfter) {
                ...
                case MT_MOVEDSTART:
                    ...
                    case MT_NOLOWERBORDERSTART:
                        // select best mark, default: use before
                        // 1960 / <5000>  long white border as closing credists
                        maxAfter = 5000;
                        break;
                    case MT_VPSSTART:
