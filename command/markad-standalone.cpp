    ...
    if (end) {
        ...
        if (end) {
            ...
            if (channelStartAfter) {
                int diffChannelStartiStopA = (iStopA - channelStartAfter->position) / macontext.Video.Info.framesPerSecond;
...
// MT_LOGOSTART ( 82878) ->   81600s -> MT_LOGOSTOP ( 84918) -> 3880ms -> MT_SOUNDSTOP ( 85015) ->  120ms -> MT_SOUNDSTART ( 85018) -> 20120ms -> MT_LOGOSTART ( 85521)
// SIIX, silence after preview (conflict)
// MT_LOGOSTART ( 35323) ->   28840s -> MT_LOGOSTOP ( 36044) ->  600ms -> MT_SOUNDSTOP ( 36059) ->   80ms -> MT_SOUNDSTART ( 36061) ->  1720ms -> MT_LOGOSTART ( 36104)
                        if ((logoStartLogoStop >= 10400) &&            // possible end of info logo before
                                ...
                                (silenceStopLogoStart >= 1840) && (silenceStopLogoStart <= 27560)) {
...
int cMarkAdStandalone::CheckStop() {
    ...
    char *indexToHMSF = marks.IndexToHMSF(iStopA, false);
    if (indexToHMSF) {
...
bool cMarkAdStandalone::MoveLastStopAfterClosingCredits(cMark *stopMark) {
    ...
    cDetectLogoStopStart *ptr_cDetectLogoStopStart = new cDetectLogoStopStart(&macontext, &criteria, ptr_cDecoder, recordingIndexMark, nullptr);
...
cMark *cMarkAdStandalone::Check_VBORDERSTART(const int maxStart) {
    ...
    if (vStop) {
        ...
        if (vNextStart || vPrevStart) {
            ...
            if ((vStart->position < IGNORE_AT_START) || ((vStart->position < iStartA) && (markDiff <= 321))) {  // vbordet start/stop from previous broadcast
    ...
    if (!criteria.LogoInBorder(macontext.Info.ChannelName)) {  // not possible for logo in border channel because vborder start and logo start can be on same position
        // valid example:
...
void cMarkAdStandalone::CheckStart() {
    ...
    char *indexToHMSFStart = marks.IndexToHMSF(iStartA, false);
    if (indexToHMSFStart) {
        ...
        if (aStart) {
            ...
            if ((macontext.Info.AspectRatio.num == 4) && (macontext.Info.AspectRatio.den == 3)) { // we have a aspect ratio start mark, check if valid
                ...
                while (aStart && (aStart->position <= (16 * macontext.Video.Info.framesPerSecond))) {    // to near at start of recording is from broadcast before
                    ...
                    aStart = marks.GetNext(aStart->position, MT_ASPECTSTART);
                    if (aStart && aStart->position > (iStartA +  (MAX_ASSUMED * macontext.Video.Info.framesPerSecond))) aStart = nullptr; // too late, this can be start of second part
                }
...
void cMarkAdStandalone::CheckStartMark() {
    ...
    if (startMark) {
        ...
        if (startMark->type == MT_LOGOSTART) {
            dsyslog("cMarkAdStandalone::CheckStartMark(): final logo start mark selection");
            ...
            if (logoStop1) {
                ...
                if (logoStart2) {
                    cMark *logoStop2 = marks.GetNext(logoStart2->position, MT_LOGOSTOP);
                    if (logoStop2) {
                        ...
                        int lengthBroadcast2    = (logoStop2->position  - logoStart2->position) / macontext.Video.Info.framesPerSecond;
                        int newDiffAfterAssumed = (logoStart2->position - iStart)               / macontext.Video.Info.framesPerSecond;
                        dsyslog("cMarkAdStandalone::CheckMarks(): MT_LOGOSTART (%4d) -> %4ds -> MT_LOGOSTOP (%4d) -> %4ds ->  MT_LOGOSTART (%4d) (%4ds after assumed) -> %4ds -> MT_LOGOSTOP (%4d)",  startMark->position, lengthBroadcast1,  logoStop1->position, lengthAd, logoStart2->position, newDiffAfterAssumed, lengthBroadcast2, logoStop2->position);
// example of valid logo start mark
// MT_LOGOSTART (7115) -> 82s -> MT_LOGOSTOP (9169) ->  0s ->  MT_LOGOSTART (9188) (66s after assumed) ->    10s -> MT_LOGOSTOP (9458) -> Comedy Central, preview direct after stop
//
// example of invalid logo start mark
// MT_LOGOSTART (2944) -> 25s -> MT_LOGOSTOP (3584) -> 50s ->  MT_LOGOSTART (4848) (193s after assumed) -> 1213s -> MT_LOGOSTOP (35173) -> sixx, start of preview before broadcast
                        if ((lengthBroadcast1 <= 105)  && (lengthAd <= 50) &&
                                (lengthBroadcast2 >= 1213) &&   // if second broadcast part is very long, this maybe the valid first part
                                (newDiffAfterAssumed <= 387)) {
                            dsyslog("cMarkAdStandalone::CheckMarks(): current start mark invalid, delete start (%d) and stop (%d) mark", startMark->position, logoStop1->position);
            ...
            dsyslog("cMarkAdStandalone::CheckStartMark(): final start mark selection");
...
void cMarkAdStandalone::AddMarkVPS(const int offset, const int type, const bool isPause) {
    ...
    if (timeText) {
        if ((((mark->type & 0xF0) >= MT_LOGOCHANGE) || (mark->type == MT_RECORDINGSTART)) &&  // keep strong marks, they are better than VPS marks
                ((mark->type != MT_TYPECHANGESTOP) || !criteria.GoodVPS(macontext.Info.ChannelName))) { // keep broadcast start from next recording if no VPS event expected
...
// preload frames before stop mark
    while (ptr_cDecoder->GetFrameNumber() <= (*mark1)->position ) {
        ...
        if ((ptr_cDecoder->GetFrameNumber() > (DEBUG_OVERLAP_FRAME_BEFORE - DEBUG_OVERLAP_FRAME_RANGE)) &&
                (ptr_cDecoder->GetFrameNumber() < (DEBUG_OVERLAP_FRAME_BEFORE + DEBUG_OVERLAP_FRAME_RANGE))) SaveFrame(ptr_cDecoder->GetFrameNumber(), nullptr, nullptr);
#endif
...
// process frames after start mark and detect overlap
    while (ptr_cDecoder->GetFrameNumber() <= fRangeEnd ) {
        ...
        if ((ptr_cDecoder->GetFrameNumber() > (DEBUG_OVERLAP_FRAME_AFTER - DEBUG_OVERLAP_FRAME_RANGE)) &&
                (ptr_cDecoder->GetFrameNumber() < (DEBUG_OVERLAP_FRAME_AFTER + DEBUG_OVERLAP_FRAME_RANGE))) SaveFrame(ptr_cDecoder->GetFrameNumber(), nullptr, nullptr);
#endif
    ...
    while (mark) {
        ...
        if ((mark->type & 0x0F) == MT_START) {
            ...
            if (soundStartBefore) {
                ...
                if (soundStopBefore) {
                    ...
                    if (lower) lowerBefore = true;
                    dsyslog("cMarkAdStandalone::SilenceOptimization(): start mark (%6d): silence from (%6d) to (%6d) %8dms before, length %4dms, black %d, lower %d", mark->position, soundStopBefore->position, soundStartBefore->position, diffBefore, lengthBefore, blackBefore, lowerBefore);
                }
            }
            if (soundStartAfter) {
                ...
                if (soundStopAfter) {
                    ...
                    dsyslog("cMarkAdStandalone::SilenceOptimization(): start mark (%6d): silence from (%6d) to (%6d) %8dms after,  length %4dms, black %d, lower %d", mark->position, soundStopAfter->position, soundStartAfter->position, diffAfter, lengthAfter, blackAfter, lowerAfter);
                }
            }
            ...
            if (soundStartBefore && (soundStartBefore->position != mark->position)) { // do not move to same frame
                ...
                case MT_LOGOSTART:
                    ...
                    if (criteria.LogoFadeInOut(macontext.Info.ChannelName) & FADE_IN) {
                        if (lengthBefore >= 120) maxBefore = 8399;
                        else                     maxBefore = 8079;
                    }
                    else                         maxBefore = 1599;
                    break;
                case MT_MOVEDSTART:
                    ...
                    case MT_VPSSTART:
                        ...
                        if (criteria.GoodVPS(macontext.Info.ChannelName)) maxBefore =  30259;
        ...
        if ((mark->type & 0x0F) == MT_STOP) {
            ...
            if (soundStopBefore) {
                ...
                const cMark *lowerStart = blackMarks.GetPrev(soundStopBefore->position, MT_NOLOWERBORDERSTOP);
                if (lowerStart) {
                    const cMark *lowerStop = blackMarks.GetNext(lowerStart->position, MT_NOLOWERBORDERSTART);
                    if (lowerStop && (lowerStop->position >= soundStopBefore->position)) lowerBefore = true;
                }
            }
            if (soundStopAfter) {
            ...
            if (soundStopAfter && (soundStopAfter->position != mark->position)) {
                ...
                if (diffAfter <= maxAfter) {
                    int newPos = soundStartAfter->position;
                    if ((diffAfter <= 40) && (lengthAfter >= 920)) newPos = soundStopAfter->position; // near and long silence is in separator after broadcast stop
...
void cMarkAdStandalone::ProcessOverlap() {
    ...
    if (time(nullptr) < (startTime+(time_t) length)) return;  // we are running during recording and has not reached end of recording
