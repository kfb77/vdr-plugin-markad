    ...
    while ((ptr_cDecoder->GetFrameNumber() < stopPosition) && status){
        ...
        if (ptr_cDecoder->isVideoIFrame()) {
            ...
            if (logo1->iFrameNumber >= 0) {  // we have a logo pair
                count++;
                int rate0 = 0;
#define AD_IN_FRAME_MIN_0 650
                if (CompareLogoPair(logo1, logo2, logoHeight, logoWidth, area->corner, AD_IN_FRAME_MIN_0, 930, &rate0)) {
                    ...
                    dsyslog("cExtractLogo::SearchAdInFrame(): corner %d frame (%5d) and frame (%5d) is different, rate %4d (expect >= 700)", area->corner, logo1->iFrameNumber, logo2->iFrameNumber, rate0);
