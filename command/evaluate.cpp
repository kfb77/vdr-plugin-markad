...
int cEvaluateLogoStopStartPair::GetIsAdInFrame(const int startPosition) {
    dsyslog("cEvaluateLogoStopStartPair::GetIsAdInFrame(): startPosition %d", startPosition);
    for (std::vector<sLogoStopStartPair>::iterator logoPairIterator = logoPairVector.begin(); logoPairIterator != logoPairVector.end(); ++logoPairIterator) {
        dsyslog("cEvaluateLogoStopStartPair::GetIsAdInFrame():: stop (%5d) start (%5d) pair: isAdInFrame %d", logoPairIterator->stopPosition, logoPairIterator->startPosition, logoPairIterator->isAdInFrame);
    }

    std::vector<sLogoStopStartPair>::iterator found = std::find_if(logoPairVector.begin(), logoPairVector.end(), [startPosition](sLogoStopStartPair const &value) ->bool { if (value.startPosition == startPosition) return true; else return false; });

    if (found != logoPairVector.end()) {
        dsyslog("cEvaluateLogoStopStartPair::GetIsAdInFrame(): IsAdInFrame for start mark (%d): state %d", startPosition, found->isAdInFrame);
        return found->isAdInFrame;
    }

    dsyslog("cEvaluateLogoStopStartPair::GetIsAdInFrame(): IsAdInFrame for start mark (%d): state not found", startPosition);
    return STATUS_ERROR;
}
...
#define AD_IN_FRAME_STOP_OFFSET_MAX  15800  // for preview direct after ad in frame changed from 15360 to 15800
#define AD_IN_FRAME_START_OFFSET_MAX  4319  // changed from 4799 to 4319
    ...
    for(std::vector<sCompareInfo>::iterator cornerResultIt = compareResult.begin(); cornerResultIt != compareResult.end(); ++cornerResultIt) {
        ...
        if ((similarCornersLow >= 3) && (similarCornersHigh >= 2)) {
            if (AdInFrameType1.start == -1) {
                AdInFrameType1.start = (*cornerResultIt).frameNumber1;
#ifdef DEBUG_MARK_OPTIMIZATION
                dsyslog("cDetectLogoStopStart::AdInFrameWithLogo(): type 1 start (%d)", AdInFrameType1.start);
#endif
            }
...
#ifdef DEBUG_MARK_OPTIMIZATION
                dsyslog("cDetectLogoStopStart::AdInFrameWithLogo(): type 1 start (%d) end (%d), isStartMark %d, startOffset %dms",  AdInFrameType1.start, AdInFrameType1.end, isStartMark, startOffset);
#endif
    ...
    if (AdInFrameType1.frameCountFinal > 0) {
        int firstFramePortionQuote  = firstSumFramePortion  / AdInFrameType1.frameCountFinal;
        ...
        if (secondFramePortionQuote <= 0) {  // best frame is a still image from recording
            dsyslog("cDetectLogoStopStart::AdInFrameWithLogo(): not enought frame pixel found on second best corner found, advertising in frame type 1 not valid");
            AdInFrameType1.startFinal = -1;
        }
    }
    ...
    for (int corner = 0; corner < CORNERS; corner++) {
        ...
        if (((logoQuote >= 894) && (framePortionQuote <  476)) || // logoQuote changed from 900 to 894, framePortionQuote changed from 500 to 476
            ((logoQuote >= 992) && (framePortionQuote <= 521))) { // high matches qoute 992, frame portion quote 521, second logo a with a frame
