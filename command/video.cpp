...
// check if we have a separation image
    if (area.status == LOGO_VISIBLE) {
        if (contrastLogo <= 6) {  // we have a very low contrast, now check full plane 1 and plane 2
...
// biuld a curve from examples:
// contrast  90, brightness 183 is valid
// contrast  30, brightness 217 is valid
// contrast  13, brightness 128 is valid     no logo with blue sky in background
// contrast  81, brightness 206 is invalid
// contrast  49, brightness 217 is invalid
// contrast  13, brightness 207 is invalid   very bright part with invisable logo
// contrast   3, brightness 206 is invalid   very bright part with invisable logo
// contrast   9, brightness 205 is invalid   very bright part with invisable logo
    ...
    if (brightnessLogo >= 227) { // this is too bright, nothing we can work with
#ifdef DEBUG_LOGO_DETECTION
        dsyslog("cMarkAdLogo::ReduceBrightness(): brightness in logo area to high");
#endif
        return BRIGHTNESS_ERROR; // nothing we can work with
    }
    if (((contrastLogo >= 81) && (brightnessLogo >= 206)) ||
        ((contrastLogo >= 49) && (brightnessLogo >= 217))) { // too much contrast in this bright area
#ifdef DEBUG_LOGO_DETECTION
...
#define MAX_AREA_INTENSITY 125  // change from 128 to 127 to 126 to 125
        if (((area.intensity > MAX_AREA_INTENSITY) || // if we found no logo try to reduce brightness
           ...
           (((area.status == LOGO_INVISIBLE) ) || ((area.status == LOGO_VISIBLE) && (rpixel < (mpixel * LOGO_IMARK))))) {
        ...
        if ((area.intensity > MAX_AREA_INTENSITY) && (contrast == BRIGHTNESS_ERROR)) {  // still too bright, reduced from 130 to 120 to 128
            return LOGO_NOCHANGE;
        }
    }
